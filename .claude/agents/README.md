...
| **`server/clones/**`** — runtime data holding a **stale copy of the entire repo**, `@devdigest/shared` **copy-vendored into two trees**, cross-package imports via **tsconfig path aliases** (this is not a monorepo), and `server/package.json` on git **`skip-worktree`** | **`investigator`'s four search traps.** Each one silently produces a confident, wrong answer: a grep that hits `clones/` returns a path that isn't real; an impact analysis naming one vendored contract copy gets someone a server/client divergence; grepping `../../reviewer-core` finds nothing because the import is `@devdigest/reviewer-core`; and a clean `git status` never proves `server/package.json` is unmodified. This is the whole reason the agent beats a plain grep |
