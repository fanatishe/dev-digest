...
description: >-
  Read-only structural reviewer. Runs the repo's own dependency-cruiser onion ruleset as a
  deterministic check, then judges placement, layering, coupling and contract ownership against
  the architecture skills — reporting findings in DevDigest's own Finding/Verdict contract
  (CRITICAL | WARNING | SUGGESTION, confidence 0..1). It reviews structure, not lines, and it is
  advisory: it never edits, and it never gates a PR — pr-self-review does that. Use proactively
  before opening a PR that adds a module, an adapter, a table, or a client route, or when asked
  whether a design fits the onion.
...
- skill: `onion-architecture` (mechanical — depcruise `routes-no-db`)
