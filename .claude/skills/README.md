...
| [run-plan](run-plan/SKILL.md) | Meta | Runs the execution half of the SDD pipeline from an approved plan — fans `implementer`s out over the work packages, runs the verifiers once writes settle, loops on `architecture-reviewer`'s advisory comments, checks the spec, runs the `pr-self-review` gate. Main-loop only (spawns subagents, asks you questions); pauses at the seams; writes nothing itself and never commits |
| [workflow-retro](workflow-retro/SKILL.md) | Meta | **Manual only.** Post-run retrospective over a multi-agent/workflow session — reconstructs the launch order, per-agent tokens/tools/duration/outcome, aggregates cost & parallelism, mines the agents' own reports for what was hard/easy/duplicated/missed, and ends with ranked recommendations. Read-only + one report; never auto-runs, never commits. Main-loop only (the usage blocks and journals are visible only to the orchestrator) |
