...
export function ConfigTab({ agent }: { agent: Agent }) {
  ...
  // Switching agents resets this form by remounting via `key={agent.id}` in
  // AgentEditor — the useState initializers above re-run on mount, so no
  // prop→state sync effect is needed here.
