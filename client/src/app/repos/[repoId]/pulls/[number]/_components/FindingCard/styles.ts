...
export const s = {
  card: (focused: boolean, sevColor: string, muted: boolean): CSSProperties => ({
    borderRadius: 8,
    // Per-side longhand only — even `borderColor` is a shorthand relative to
    // `borderLeftColor`, so React warns when one updates on rerender. Set each
    // side's colour explicitly instead.
    borderStyle: "solid",
    borderWidth: 1,
    borderLeftWidth: 3,
